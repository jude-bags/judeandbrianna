...
export default function RSVPForm() {
  ...
  const handleRadioChange = (value: string, name: string) => {
    // Reset guest info if attending changes to no
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white/80 backdrop-blur-sm p-8 md:p-12 w-full max-w-2xl mx-auto">
      ...
      {/* Display declined message and buttons if attending is no */}
      {formData.attending === 'no' && (
        <div className="mb-12 p-6 border border-wedding-gray-300 bg-wedding-gray-100">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link 
              to="/registry" 
              className="uppercase px-6 py-3 border border-wedding-black hover:bg-wedding-black hover:text-white transition-colors duration-300 text-sm tracking-widest font-medium text-center"
            >
              REGISTRY
            </Link>
            <Link 
      ...
      {/* Bringing Guest Question - shown only when attending */}
      ...
      {/* Guest Name Fields - shown when bringing a guest */}
      ...
      {/* Food Restrictions - only shown when attending */}
      {formData.attending === 'yes' && (
        <div className="mb-16">
          <div className="flex items-center mb-6">
